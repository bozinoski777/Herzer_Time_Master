...
10. Rebuilds the worker D3 `Standort` select from currently `Active = true` D8 sites plus the five standard work choices, retaining the legacy `Teil-Tag` option only where it already exists. This happens only after the old month is safely hidden; historic D4 and D7 options are never pruned.
...
| `TWILIO_ACCOUNT_SID` | The Account SID (`AC...`) for the working US1 Twilio account. Required only by the SMS reminder workflow. |
| `TWILIO_AUTH_TOKEN` | The live Auth Token for that account; never commit or print it. |
| `TWILIO_MESSAGING_SERVICE_SID` | The Messaging Service SID (`MG...`) in the same account, with its approved sender pool. |
