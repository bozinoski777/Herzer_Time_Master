...
To diagnose **401 / 20003**, open **Actions → SMS reminders for incomplete time entries → Run workflow**, enable `check_credentials_only`, and choose `auth_mode = auth-token` to bypass a stale API key without deleting its secrets. This check can run on any date, sends no SMS, and does not read or update Notion. It checks Messages read access and prints only the credential type, region, and safe error details; success does not prove sender configuration or delivery. Standard API keys support this check; a restricted key must have Messages read permission. The check is optional and does not impose read permission on normal SMS sends. For a local check with environment variables already configured, run `npm run check-twilio`. Once the check passes, start a new manual run with `check_credentials_only` disabled and the intended `target_worker` filled in.
