...
| `TWILIO_AUTH_TOKEN` | The live Auth Token for that account and region; never commit or print it. Used in `auth-token` mode or as the `auto` fallback after an explicit API key authentication rejection. |
...
For paid Twilio production use, create a **live Standard API Key** in the same Twilio account and region as the Messaging Service and save its `SK...` SID and one-time secret as `TWILIO_API_KEY_SID` and `TWILIO_API_KEY_SECRET`. The API key secret is different from the account Auth Token. In the default `auto` mode, the workflow prefers this pair; if Twilio explicitly rejects it with **HTTP 401 / code 20003**, it tries the configured Auth Token once and uses that credential for the remaining workers. It never switches credentials after a timeout, network failure, other HTTP error, or ambiguous response. Create a **Messaging Service** in Twilio, add an approved sender to its Sender Pool, and save its `MG...` ID as `TWILIO_MESSAGING_SERVICE_SID`. For German one-way reminders, Twilio supports a dynamic alphanumeric sender ID such as `HERZER`; it must be configured in that Messaging Service and can only be used on a paid account. The workflow then sends `MessagingServiceSid` rather than selecting a sender itself. Workers must have agreed to receive these messages, and the text includes a non-SMS route to opt out through management.

Two optional **Actions repository variables** (the Variables tab, not secrets) control authentication:

| Variable | Value |
| --- | --- |
| `TWILIO_AUTH_MODE` | `auto` (default), `api-key`, or `auth-token`. Explicit modes use only the selected credential. A manual run can override this through `auth_mode`; `repository-default` preserves the repository setting. |
| `TWILIO_REGION` | `us1` (default) or `ie1`. The account credentials and Messaging Service must belong to the selected region. IE1 uses `api.dublin.ie1.twilio.com`, as required by [Twilio's regional Messaging API](https://www.twilio.com/docs/global-infrastructure/messaging-api-with-twilio-regions). |

To diagnose **401 / 20003**, open **Actions → SMS reminders for incomplete time entries → Run workflow**, enable `check_credentials_only`, and choose `auth_mode = auth-token` to bypass a stale API key without deleting its secrets. This check can run on any date, sends no SMS, and does not read or update Notion. It checks Messages read access, checks account status when using an Auth Token, and verifies that the configured Messaging Service is accessible and belongs to TWILIO_ACCOUNT_SID. Standard API keys skip the Accounts lookup because they cannot read that resource. The check prints only safe status summaries; success does not prove sender-pool readiness, send permission, or delivery. A restricted key needs Messages and Services read permissions for these diagnostics. If Messages authentication passes but the service lookup fails, inspect the Messaging Service SID, account ownership, region, and read permissions instead of replacing otherwise working credentials. The check is optional and does not impose read permission on normal SMS sends. In US1, it also looks for recent 20003 Message POST errors in the latest 100 error records from the last 24 hours, printing up to five redacted historical descriptions. Error-history access is optional and does not change the result of the configuration checks. For a local check with environment variables already configured, run `npm run check-twilio`. Once the check passes, start a new manual run with `check_credentials_only` disabled and the intended `target_worker` filled in.

For the first live check, add the Twilio secrets above, fill `SMS Telefonnummer` for one designated test worker, then open **Actions → SMS reminders for incomplete time entries → Run workflow**. In `target_worker`, enter that worker's exact D1 `Worker Key` (preferred) or exact `Vor- und Nachname`; only that active, ready worker can be selected. Leave `target_worker` blank only when you deliberately want to check all eligible workers. A Twilio trial can send only to verified recipient numbers and predefined templates; upgrade before expecting this custom employee reminder to deliver. The workflow does not print phone numbers, SMS text, Auth Tokens, or raw Twilio API responses.
