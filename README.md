...
- **Standort sync** adds active D8 sites and work choices to each ready worker's D3, and removes a deactivated site only when no current D3 day still uses it. D4 Archiv and D7 retain every historic option. New work choices are created gray; the Notion API does not allow an existing select option's color to be changed, so existing colors are preserved safely. It is scheduled ten minutes after daily sync and waits on the shared safety lock if another Notion workflow is active.
...
| Standort sync | `27 6,14 * * 1-5` | 06:27 and 14:27 UTC on UTC weekdays |
