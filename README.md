...
- **Daily sync** appears in GitHub as two connected stages. **1 · Upsert worker days into management** copies D3 to D7 using `Worker Key|Datum` as the D7 `Sync Key`, so each source day is created once or updated idempotently. Blank values are also written, so D7 reflects a worker's corrections instead of retaining stale data. **2 · Sync active D3 Standort options** adds active D8 sites and work choices to each ready worker's D3, and removes a deactivated site only when no current D3 day still uses it. D4 Archiv and D7 retain every historic option.
